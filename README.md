...
#### 메이커 (시나리오 제작)
- 5단계 Step Wizard (기본 설정 → 사건 개요 → 플레이어 → 장소/단서 → 스크립트)
- 기본 설정: 제목·인원·난이도·테마·분위기·예상 시간
...
- [ ] **기본 설정 편집 불가 문제**: 메이커 Step 1이 편집 모드에서 "수정 불가" 안내만 표시됨.
  `SettingsForm`을 편집 모드에서도 사용 가능하게 하거나 별도 편집 UI 추가 필요.
...
- [ ] **라운드 중 장소 현황 UI (GM 화면)**: 라운드 진행 중 GM 화면에
  장소별 총 단서 수 / 남은 단서 수 표시. 장소 이미지가 있으면 지도형 레이아웃 지원.
