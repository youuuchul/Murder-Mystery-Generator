...
- GM 화면 개선
  - 라운드 진행중 GM BOARD 아래 장소 현황 및 잔여 카드현황 표기
  - ~~오프닝, 엔딩에서 공통 이미지/지도 삭제~~ (2026-03-13 완료)
- 제작 화면 전반
  - ~~제작 네비게이터의 필수 입력/단서 부족 검증 힌트 UI~~ (2026-03-13 완료)
  - llm 기반 시나리오 제작 도우미 에이전트 챗봇 (타임라인/단서 모순 검토, 배경 및 설정 기반 추가 단서카드 제안 등)
- 제작 화면: 페이즈 별 입력
  - ~~제작 화면: 페이즈 별 진행 가이드 흑백 예시 -> 입력이 기본값 / 작성하세요 안내(흑백 예시)~~ (2026-03-13 완료)
- 제작 화면 - 플레이어 탭
  - ~~알리바이 별도 입력 칸 제거~~ (2026-03-13 완료)
  - ~~플레이어 칸 전원 공개 배경 / 상세 스토리 분리 후 추가칸 생성 : 스토리는 길어져야하니 입력 토글형태 고려~~ (2026-03-13 완료)
- 단서 카드 ui
  - 단서 카드 이미지 입력 지원 및 인벤토리 오픈 시 이미지 함께 보이는 ui (카드처럼)
- 플레이어 ui
  - 플레이어의 캐릭터 배경/타임라인 ui 추가 개선

## 6. 후속 진행 메모

- 2026-03-13 작업에서 위 완료 항목들이 실제 구현됨
- 최신 세부 내역과 남은 우선순위는 `20260313_2200_MakerValidation_PlayerFlow_Report.md` 참고
