...
- 후속 채팅이 `previous_response_id` 에 의존하지 않도록 변경했다.
- 최근 대화 8턴을 직접 prompt에 포함해 연속 질문이 끊기지 않게 했다.
- 요청 실패 시 막 보낸 유저 메시지를 채팅 목록에서 제거해, 에러 턴이 다음 프롬프트 문맥을 오염시키지 않게 했다.
- `draft` 문체를 아래 3가지 프로필로 세분화했다.
  - `narrative_prose`
  - `descriptive_copy`
  - `gm_guide`
- 오프닝/배경/엔딩 같은 서사형 요청은 게임 설명문이 아니라 자연스러운 산문으로 쓰도록 프롬프트를 강화했다.
- 플레이어 수, 단서 개수, 타임라인 시각, 라운드 번호, 해금 조건 같은 설계 메타를 본문에 그대로 쓰지 않도록 금지 규칙을 넣었다.
...
- 스토리형 문안에서 여전히 남아 있는 기계적 사실 나열을 더 줄이는 후속 튜닝
