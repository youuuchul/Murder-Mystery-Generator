...
- `chat` 요청에 대해 `guide/draft` 모드 분리가 구현됐다.
- 사용자는 메이커 도우미 입력창에서 `자동 / 가이드 / 문안` 모드를 직접 고를 수 있다.
- `자동` 모드는 서버에서 질문 문장 키워드를 보고 `guide` 또는 `draft` 를 추론한다.
- `draft` 응답은 `title/body/notes` 구조로 파싱되고, UI에서도 분석 카드와 분리된 “붙여넣기용 본문” 카드로 렌더링된다.
- 빠른 액션(`모순 점검`, `단서 제안`, `다음 작업 추천`)은 여전히 `guide` 응답으로 고정한다.
- 추가로 assistant step 참조 범위가 `1~6` 으로 정리되어 Step 6 엔딩 편집 상황도 정상 처리한다.

### 이번 1차에서 아직 남은 것

- 특정 입력칸에 바로 넣는 `복사` 버튼
- Step별 세부 템플릿 고도화
- quick action 쪽의 `draft` 프리셋 버튼 분리 여부 검토
- `draft` 결과를 메이커 폼 필드에 직접 주입하는 UX
