...
> 마지막 업데이트: 2026-04-16 (범인 지정에 피해자/NPC 허용 + 투표 `players-and-npcs` 모드에 피해자 자동 포함, 신규 게임 생성 페이지를 제목+소개글만 받는 얇은 셸로 단순화, 편집 화면 저장/네비 액션바를 드래그 가능한 플로팅 바로 전환 + AI 런처 겹침 회피 + 콘텐츠 하단 여백 자동 확보, 모바일 뷰포트 meta 누락 수정, iOS input 포커스 자동 확대 차단, 캐릭터 카드 승리조건 배치 정돈, 게임 커버 GM/플레이어 버튼 정책 정리, GM 전용 진행 가이드(gmNote) 폐지, 퍼스널/과거 기록 git 추적 제외 + set-maker-role.mjs Supabase 전용 단순화, 장소 character_has_item 조건을 "열기" 액션 + sky-pill 알림으로 전환)
...
### 🚨 긴급
- [x] **범인 셀렉트 빈 카테고리도 항상 노출** — 직후 follow-up. 피해자/NPC 가 게임에 없으면 카테고리 자체가 숨겨져 사용자가 "어디서 추가하지?" 싶어 길을 잃던 문제. 플레이어/피해자/NPC 3 optgroup 을 항상 그리고, 비어 있을 땐 "사건 개요 탭에서 추가하세요" disabled 안내 옵션을 노출. 설명 문구도 투표 탭의 "플레이어 + NPC + 피해자" 옵션과 같은 후보군임을 명시. ✅ 완료 (2026-04-16)
