...
- [ ] **메이커 UI 정보 밀도/중복 카드 정리** — 메이커 편집 화면 전반에 카드형 안내·상태·요약 정보가 과하게 쌓여 실제 입력 영역보다 보조 정보가 더 크게 보이는 문제. 특히 장소/단서/접근 조건 영역에서 같은 의미가 제목·상태 pill·설명문·카드 본문에 반복됨. 예시: "접근 불가 캐릭터 (소유자 — 자기 공간에는 들어갈 수 없음)", "겜블러 선택", "겜블러은(는) 이 장소에서 단서를 획득할 수 없습니다."처럼 동일 제약을 여러 줄로 보여주는 중복. 작업 방향: (1) 각 Step별 카드/패널/상태 pill 목록 작성, (2) 정보 유형을 입력 필드·검증 오류·운영 상태·도움말로 분류, (3) 동일 의미 반복 제거, (4) 필수 상태만 접힘 없는 위치에 남기고 보조 설명은 접힘/tooltip/empty state로 이동. 우선 진입점: `LocationEditor.tsx`, `PlayerEditor.tsx`, `VoteEndingEditor.tsx`, `MakerEditor.tsx` 이슈 패널.
