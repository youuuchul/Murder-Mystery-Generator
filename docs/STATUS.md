...
> 마지막 업데이트: 2026-04-11 (투표&엔딩 재설계 완료 — 퍼널 분리, 다중 선택지 매핑, 2차 투표 API)
...
- [x] **투표 & 엔딩 고급 시스템** — 메이커 투표 탭 개선(기본 투표 질문 텍스트/선택지 옵션/투표 전 텍스트 on/off, 2차 투표 명칭 변경). 메이커 엔딩 탭 다중 선택지→분기 매핑 UI. 플레이어 퍼널 4단계 분리(투표결과→분기엔딩→개인엔딩+점수→작가후기). 2차 투표 API 구현(제출/결과공개/분기resolve). EndingStage 확장(vote-result, vote-round-2-pre-story 추가). 개인 엔딩 레거시 마이그레이션 안내. **후속 필요**: 추가 투표 캐릭터별 분리, 승리 점수 계산 연동 ✅ 완료 (2026-04-11)
...
- [ ] **플레이어 화면 스포일러 방지 개선** — (1) 엔딩 화면: 범인 검거 성공/실패 결과가 상단에 바로 보여서 텍스트 읽기 전에 스포일러. 결과를 스크롤 하단이나 점진 공개로 이동. (2) 캐릭터 카드: 승리 조건이 상단에 크게 노출되어 게임 중 카드 다시 볼 때 다른 사람에게 들킬 위험 + 몰입 저해. 접힘 처리나 하단 배치 검토.
- [ ] **메이커 단서 추가 버튼 위치 개선** — 현재 '+단서 추가'가 장소 카드 상단에만 있어서, 빈 장소에 처음 추가할 때는 좋지만 연속 추가 시 매번 위로 스크롤해야 함. 단서 목록 하단에도 추가 버튼 배치하거나, 추가 후 자동 스크롤 검토. 연속 추가 빈도가 높지 않으면 현행 유지도 가능.
- [ ] **단서 유형(type) 옵션 리워크** — 현재 physical/testimony/scene 3종은 제작자에게 불필요한 선택지. 두 가지 방향 검토: (a) 유형 필드 자체를 삭제하고 단순화, (b) 유형을 게임플레이 동작 기반으로 재설계 — 예: "전체 공개(현장 단서)", "획득 후 소유", "확인 후 반납(소유 불가)" 등 실제 동작을 명시. 메이커 Step4 UI, game_clues.type 컬럼, 플레이어 인벤토리 로직, AI 컨텍스트에 영향. 선택 후 마이그레이션 필요.
