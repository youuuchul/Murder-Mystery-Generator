...
> 마지막 업데이트: 2026-04-30 (기본설정 예상 소요 시간/규칙 재배치)
...
- [ ] **GM 없는 세션 공통화면 범위 보강**
- [x] **예상 소요 시간 자동 요약 + 수동 덮어쓰기** — 메이커 Step1 `예상 소요 시간`을 오프닝 / 라운드 수·조사·토론 / 투표·엔딩 3블록 입력으로 재배치. 긴 슬라이더와 `타이머 합계`, `투표/엔딩은 별도`, `표기 여유` 뱃지를 제거하고, 기존 `games.estimated_duration`은 유저에게 보이는 총 예상 시간으로 유지한다. 투표·엔딩 시간은 새 DB 컬럼을 만들지 않고 `estimatedDuration - (오프닝 + 라운드 타이머 합)`으로 파생해 입력하며, 오프닝/라운드/페이즈 값을 바꿀 때 이 값을 보존해 총 예상 시간이 함께 갱신된다. 밀담은 플레이어 수 카드 안으로 이동, 카드 주고받기는 장소 & 단서 카드 탭의 단서 획득 규칙으로 이동. ✅ 완료 (2026-04-30)
