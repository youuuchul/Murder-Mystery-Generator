...
> 마지막 업데이트: 2026-04-28 (이미지 variant 경로 추가 점검 — 공통 이미지 display 명시, 투표 후보 소형 인물 이미지 thumb 요청으로 보정)
...
- [ ] **메이커 UI 정보 밀도/중복 카드 정리** — 메이커 편집 화면 전반에 카드형 안내·상태·요약 정보가 과하게 쌓여 실제 입력 영역보다 보조 정보가 더 크게 보이는 문제. 특히 장소/단서/접근 조건 영역에서 같은 의미가 제목·상태 pill·설명문·카드 본문에 반복됨. 예시: "접근 불가 캐릭터 (소유자 — 자기 공간에는 들어갈 수 없음)", "겜블러 선택", "겜블러은(는) 이 장소에서 단서를 획득할 수 없습니다."처럼 동일 제약을 여러 줄로 보여주는 중복. 1차(2026-04-28): `LocationEditor.tsx` 장소/단서 영역에서 장소 설정 설명문, 접근 제한 중복 문구, 조건 설명, 공유 단서 3줄 안내, 통계 카드 보조문을 축약. 남은 작업: PlayerEditor/VoteEndingEditor/MakerEditor 이슈 패널 및 전체 설명문 전수 정리.
- [ ] **메이커 기능 설명투/당연한 안내 문구 삭제** — 제작자에게 실제 의사결정이나 입력을 돕지 않는 설명 문구가 화면 정보량을 늘리고 있음. 예시: "이 장소의 이름, 공개설명, 개방 시점을 정합니다."처럼 바로 아래 필드 라벨만 봐도 알 수 있는 문장, 구현 의도나 기능 설명에 가까운 문장, 사용자가 굳이 알아야 하지 않는 내부 정책 설명. 작업 방향: (1) 메이커 화면의 `text-dark-500/600` 설명문 전수 검색, (2) 문구를 "사용자가 지금 결정해야 하는 것" 기준으로 유지/삭제/축약 분류, (3) 검증 실패·위험·비가역 액션 안내만 명시적으로 남김, (4) 사용자 노출 문구에서 구현자 시점/기능 설명투 제거. 1차(2026-04-28): `SettingsEditor.tsx` 기본 설정 탭의 반복 설명문, 성공 상태 카드, 빈 상태 문구를 축약하고 `PlayerEditor.tsx`의 임시 구현 문구 1건을 사용자 문구로 교체. 추가(2026-04-28): `ImageAssetField.tsx` 공통 이미지 필드에서 권장 비율/해상도/WEBP 용량/최적화 결과 뱃지와 기술 문구 제거. 남은 작업: PlayerEditor/VoteEndingEditor/MakerEditor/ScriptEditor 설명문 전수 정리.
- [ ] **메이커 연관 단서 선택 목록 정렬/탐색성 개선** — 캐릭터 상세의 `연관 단서` 드롭다운을 펼쳤을 때 단서 목록이 생성순/장소순/캐릭터별 맥락으로 정리되지 않아 원하는 단서를 찾기 어렵다. 현재 예시처럼 `지하실`, `이야기꾼의 방`, `테러리스트의 방`, `겜블러의 방`, `탐정의 방` 단서가 긴 기본 select 목록에 섞여 보이고, 방/소유자/생성순 기준이 명확하지 않음. 작업 방향: (1) `PlayerEditor.tsx`의 연관 단서 select 옵션 생성 로직 확인, (2) 기본 정렬을 장소 순서 → 단서 sortOrder/생성순으로 안정화, (3) 가능하면 장소별 `optgroup` 또는 검색 가능한 커스텀 선택 UI로 교체, (4) 이미 선택된 단서와 미선택 단서를 시각적으로 구분. 우선순위는 UI 정보량 정리와 함께 Step3 캐릭터 편집 UX 개선 묶음으로 처리.
- [ ] **메이커 관계 탭 중복 대상 선택 방지** — Step3 캐릭터 상세의 관계 탭에서 이미 관계 1~N에 선택한 대상이 다른 관계 행 드롭다운에도 계속 남아 중복 관계를 만들기 쉬움. 현재 `PlayerEditor.tsx`의 `filteredRelationTargets`는 현재 캐릭터 본인(`player:${player.id}`)만 제외하고, `player.relationships` 안에서 이미 선택된 target은 제외하지 않음. 작업 방향: (1) 각 관계 행의 현재 선택값은 드롭다운에 유지, (2) 다른 관계 행에서 이미 선택된 `targetType:targetId`는 후보에서 숨김, (3) 피해자/NPC/player 모두 같은 중복 기준 적용, (4) 기존 데이터에 중복 관계가 있으면 UI에서 경고하거나 저장 전 검증. 우선 진입점: `src/app/maker/[gameId]/edit/_components/PlayerEditor.tsx` 관계 탭.
