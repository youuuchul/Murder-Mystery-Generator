...
- [x] 카드 주고받기 불가 게임의 플레이어 카드 양도 버튼 숨김 — 서버 API는 이미 `game.rules.cardTrading.enabled=false`에서 양도를 403으로 막고 있었지만, 플레이어 카드 상세 모달에는 `이 카드 양도하기` 버튼과 확인 폼이 계속 노출돼 뒤늦게 실패 알림이 떴음. `PlayerView.tsx`에서 같은 설정값을 읽어 카드 주고받기 불가 게임에서는 양도 UI 자체를 렌더하지 않도록 수정. 레거시 데이터는 기존처럼 설정값이 없으면 허용으로 간주. ✅ 완료 (2026-04-30)

### 이미지/미디어
- [x] 업로드 전 압축/리사이즈/WEBP 변환 1차 구현 — 원본보다 커지는 변환 케이스는 후속 백로그로 분리
- [x] 이미지 권장 해상도/비율 안내
- [x] 표지 포지션 조절
- [x] 표지 크롭/확대 조정 — `games.cover_image_position_x/y`와 `cover_image_zoom`을 원본으로 사용. 메이커 기본설정에서 라이브러리 카드 기준 16:10 대형 미리보기, 좌우/상하/확대 컨트롤을 제공하고 라이브러리 카드·공개 상세 화면도 같은 렌더 helper를 사용.
...
- [x] 기본설정 태그 UI 정리 — `games.tags text[]` 저장 구조는 유지하고, 선택된 태그/직접 입력을 먼저 보이게 배치. 추천 태그 전체 노출은 접힘 영역으로 이동하고 이미 선택한 태그는 추천 목록에서 숨김. 메이커 AI 컨텍스트의 `gameSummary.tags`에도 같은 값을 전달.
- [x] 기본설정 플레이어 수/표기 난이도 UI 정리 — `games.player_count` 저장값을 1~15명으로 보정하고, 플레이어 수 안내 문구 제거. 난이도는 3단계(`easy/normal/hard`) 유지, 메이커 라벨을 `표기 난이도`로 바꾸고 부가 설명 문구 제거.
- [x] **기본설정 시간/인원 입력 컨트롤 통합** — `NumberInputField`/`TimeInputField` 두 컴포넌트를 단일 `StepperInput`(±step 버튼 + `draft` state 직접입력)으로 교체하고 5분/횟수 빠른선택 chip 제거. 인원·밀담·난이도를 3카드로 분리(`lg:grid-cols-3 items-stretch`)해 박스 높이 통일, 밀담 카드 라벨/토글 정렬을 인원·난이도 카드와 일치. 예상 소요 시간 영역에 `PhaseProportionBar`(오프닝/라운드/투표·엔딩 분 비율 segment) 추가. 데이터 모델·DB 컬럼·정규화·메이커 어시스턴트 컨텍스트 변경 없음. ✅ 완료 (2026-04-30)
...
- [x] **표지 이미지 줌 크롭 지원** — `cover_image_zoom numeric(4,2) default 1.00` 컬럼 추가. `CoverImagePosition.zoom` 타입/normalizer/repository 매핑, 라이브러리 카드·게임 상세·메이커 미리보기 렌더를 같은 helper로 갱신. ✅ 완료 (2026-04-30)
...
- [ ] **GM 없는 세션 공통화면 범위 보강**
- [x] **예상 소요 시간 자동 요약 + 수동 덮어쓰기** — 메이커 Step1 `예상 소요 시간`을 오프닝 / 라운드 / 투표·엔딩 3블록 입력으로 재배치. 긴 슬라이더와 `타이머 합계`, `투표/엔딩은 별도`, `표기 여유` 뱃지를 제거하고, 기존 `games.estimated_duration`은 유저에게 보이는 총 예상 시간으로 유지한다. 투표·엔딩 시간은 새 DB 컬럼을 만들지 않고 `estimatedDuration - (오프닝 + 라운드 타이머 합)`으로 파생해 입력하며, 오프닝/라운드/페이즈 값을 바꿀 때 이 값을 보존해 총 예상 시간이 함께 갱신된다. 2026-04-30 후속: 라운드 입력에 `통합` / `조사·토론 별도` 전환을 추가하고, 시간 입력은 브라우저 기본 증감 버튼 없는 직접 숫자 입력 + 같은 박스 안의 5분 단위 빠른 선택(최대 60분)으로 변경. 통합 모드는 `discussion.durationMinutes=0`으로 저장되어 실제 세션 진행에서도 토론 페이즈가 제외된다. 밀담은 플레이어 수 카드 안으로 이동, 카드 주고받기는 장소 & 단서 카드 탭의 단서 획득 규칙으로 이동. ✅ 완료 (2026-04-30)
...
- [ ] **토론(공개 발언) 로깅 인프라** — 머더미스터리는 오프라인 + 모바일 카드/인벤토리 구조라 지금까지 공개 발언을 텍스트로 저장할 인프라가 없음(밀담만 슬롯별 `conversationHistory`로 저장). 토론 페이즈 정의(`subPhase: "discussion"`)와 시간 설정(`phases.discussion.durationMinutes`)은 있지만 발언 입력 UI/저장/Realtime 이벤트 모두 0. 이 부재가 **AI 플레이어 페르소나 Phase 4(다른 플레이어 발언을 AI 컨텍스트로 인테이크)를 직접 차단**하므로 선결 작업. 데이터 모델: `SharedState.discussionLog: DiscussionMessage[]`(JSONB 확장, DB 스키마 변경 없음). API: `POST /api/sessions/[sessionId]/discussion`. Realtime: `discussion_message` 이벤트 추가. UI: 모바일 토론 페이즈 하단 발언창 + 최근 5건 리스트, GM 발언 흐름 패널. 입력 방식 결정 옵션 A(모바일 텍스트 채팅, MVP 권장) / B(GM 요약 타이핑) / C(음성 인식, 비용·품질 미검증) / D(AI 있을 때 강제). 현재 선호: A안 + AI 1명 이상이면 자동 ON 안내. 보존 정책은 라운드별 LLM 요약 + 하드 cap 100개. MVP = Phase 1(데이터+API+Realtime) + Phase 2(모바일 UI) = 1.5일. GM 패널/자동 발언은 분리. 상세 + 옵션 비교: `docs/plans/20260430_DISCUSSION_LOG_PLAN.md`.
- [ ] **AI 플레이어 페르소나/컨텍스트 블럭 재구성** — AI 플레이어가 캐릭터 정보(배경/스토리/단서)는 어느 정도 참조하지만 캐릭터성·말투를 살리지 못해 모든 AI가 비슷한 톤으로 수렴하는 문제. 근본 원인 3가지: (1) `Player` 타입에 말투/성격/어휘/감정 베이스라인 같은 명시적 페르소나 필드가 없음 — `background`/`story`는 "무엇을 했는가"를 적는 칸이라 "어떻게 말하는가"가 안 들어감, (2) `buildChatSystemPrompt`(`src/app/api/sessions/[sessionId]/chat/route.ts:199`)가 정적 캐릭터 정보와 동적 게임 상태(현재 라운드, 새로 받은 단서, 공개된 정보)를 한 system 메시지에 평탄하게 섞어 모델이 우선순위를 못 잡음, (3) 단서 컨텍스트가 "현재 보유 목록"만 있고 "초기 보유 vs 라운드 중 획득"이 분리 안 됨, (4) 본인이 다른 밀담에서 한 발언을 기억 못 해 모순 발생. 해결: 컨텍스트를 4개 층(L1 WORLD STATIC / L2 CHARACTER STATIC / L3 SESSION DYNAMIC / L4 CONVERSATION)으로 분리하고 메시지 배열을 `[static system, dynamic system, ...history]` 구조로 재편 — STATIC 분리로 OpenAI prompt caching hit률도 같이 노림. `Player.persona`(speechStyle/personalityTraits/vocabulary/emotionalBaseline 4 string 필드) 추가 + 메이커 입력 카드 + AI 도우미 자동 제안. 단서 인벤토리에 `acquiredAtRound`/`source` 메타 추가해 동적 블럭에서 "초기 보유 / 이전 라운드 / 이번 라운드 신규" 3분류 표기. MVP = Phase 1(프롬프트 빌더 분리, DB 변경 없음) + Phase 2(persona 필드 + 메이커 UI), 2일 규모. Phase 3(단서 라운드 메타) / Phase 4(공개 정보 + 토론 발언 — `토론(공개 발언) 로깅 인프라` 백로그 + `docs/plans/20260430_DISCUSSION_LOG_PLAN.md` 선행 필요) / Phase 5(자기 일관성 메모리) / Phase 6(캐싱 측정)은 분리 진행. 측정: Langfuse trace에 페르소나 일관성(LLM-as-judge) / 자기 일관성(모순 검출) / cached_tokens 비율 metadata 기록. 상세: `docs/plans/20260430_AI_PLAYER_PERSONA_PLAN.md`. **UI 위치 확정(2026-05-04)**: 메이커 Step 3 캐릭터 카드 sub-tab으로 [캐릭터 정보 / 타임라인 / **AI 페르소나**] 같은 레벨에 추가. 진입점 `PlayerEditor.tsx`의 `view: "info" | "timeline"` toggle을 3-tab으로 확장. Step 3 description은 이미 `캐릭터 정보·타임라인·AI 페르소나`로 갱신되어 있음.
