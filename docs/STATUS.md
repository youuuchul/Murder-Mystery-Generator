...
- [x] **단서 획득 액션 응답 지연 개선** — (1) AI auto-acquire 후 Langfuse 트레이스를 직렬 await하던 코드 제거 — 결정론 로직이라 트레이스 불필요. AI N명 × RTT 가산 제거. (2) acquire 응답에 `sharedState/inventory/roundAcquired/roundVisitedLocations` 동봉 → SSE/폴링 대기 없이 클라가 즉시 setState. (3) `try/finally`로 버튼 잠금 해제 보장. (4) `shared_clue_discovered` + `clue_acquired` 이벤트 실시간 알림 — 초기엔 SSE로 구현했으나 Vercel 서버리스에서 in-memory broadcaster가 다른 instance에 닿지 못해 유실되는 문제 발견 → **Supabase Realtime Broadcast로 전환** (REST /realtime/v1/api/broadcast 발행 + 브라우저 anon 구독). 상단 floating pill 알림(3s) + SSE 수신 시 폴링 backoff(2.5s). AI 자동 획득 outcome도 동일 이벤트로 브로드캐스트. "플레이어(캐릭터)" 포맷. (5) `alert()` 제거 → inline 토스트. 409 "이미 다른 플레이어 보유" 시 sharedState 낙관적 push로 버튼 즉시 `takenByOther` 전환. ✅ 완료 (2026-04-14)
