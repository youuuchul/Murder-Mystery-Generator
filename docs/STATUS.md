...
- [ ] **개인 투표(vote-answer) 작성 흐름 통합 — 캐릭터 탭 인라인 모달** — 사용자 보고(2026-04-27): 현재 캐릭터의 `vote-answer` 승점 조건을 만들려면 (1) Step3 캐릭터에서 승점 조건 추가→`vote-answer` 선택해보면 "투표 탭에서 먼저 추가하세요" 안내에 막힘 → (2) Step6 투표 탭으로 이동해 `purpose:"personal"` 질문+선택지 생성 → (3) 다시 Step3로 돌아와 questionId 선택+정답(`expectedAnswerId`) 지정. 한 의미 단위(이 캐릭터의 개인 투표)에 화면 3번 왕복. 원인: `Player.scoreConditions[i].config.questionId` ↔ `VoteQuestion.personalTargetPlayerId` 양방향 ref 구조라 어느 쪽에서 시작해도 반대편이 선행돼야 닫힘. 추천 방안 A(인라인 모달): 캐릭터 탭의 `ScoreConditionsEditor`에서 `vote-answer` 분기를 "+ 이 캐릭터의 개인 투표 만들기" 버튼으로 바꾸고 모달 한 화면에서 질문 라벨/타깃 모드/선택지/정답까지 입력. 저장 시 `voteQuestions` 배열에 push + `personalTargetPlayerId=player.id` 자동 + `ScoreCondition.config.{questionId, expectedAnswerId}` 채움. 기존 personal 질문이 있으면 "기존 질문 연결" 드롭다운도 노출. Step6 투표 탭은 그대로 두되 personal 질문 카드에 "X 캐릭터 승점과 연결됨" 라벨 + 캐릭터 점프 버튼 추가(단일 진실 원천: `voteQuestions`). 무결성 가드: personal 질문 삭제 시 "X 캐릭터 승점 미완성" 경고, 캐릭터 삭제 시 전용 personal 질문 자동 정리. 진입점: `src/app/maker/[gameId]/edit/_components/PlayerEditor.tsx:1037 ScoreConditionsEditor`, `VoteEndingEditor.tsx`, `types/game.ts:155 ScoreConditionType` / `404 VoteQuestion`. DB 마이그레이션 불필요(스키마 변동 없음, UI 재배치만). 발견: 2026-04-27.
- [ ] **메이커 제작 상태/카운트 시스템 잔해 정리** — 최근 수개월간 메이커 스텝 구조·필드·검증 규칙이 여러 차례 바뀌면서(Step 6단계화, 타임라인 inactive 플래그, gmNote 폐지, custom-choices 분기, NPC/피해자 범인 확장 등) 제작 상태 표시 영역에 정합 깨진 부분이 누적된 것으로 보임. 사용자 보고(2026-04-27): "제작상태, 입력된 항목들 카운트 같은 곳에 잔해가 남아있거나 동작 안 되는 게 많아 보인다." 진단 범위: (1) `src/lib/maker-validation.ts` 의 모든 누락 카운트 규칙(타임라인 inactive 반영은 됐으나 다른 신규 필드/모드 케이스는 미반영 가능성), (2) `MakerEditor.tsx` 사이드바·헤더·플로팅 액션바의 "남은 항목/완성도" 류 표시 컴포넌트, (3) 라이브러리 카드 `buildMetadataFromRow` lifecycle/readiness 표시(2026-04-15 한 차례 수정한 fallback 로직 외 다른 항목 잔존 가능성), (4) `game-publish.ts` 공개 전 체크리스트와 메이커 내부 검증의 룰 일치 여부, (5) 이슈 패널/오류 위치 점프 동작이 신규 추가 필드(victim/NPC 범인, custom-choices, advancedVoting, 타임라인 inactive 등)를 인식하는지. MVP 작업: (a) 검증 규칙 일람을 표로 추출(코드 그루핑 → 어느 필드가 어디서 어떻게 카운트되는지 1:1 매핑), (b) 더이상 안 쓰는 룰/필드 제거, (c) 신규 필드 누락 케이스 보강, (d) 라이브러리·관리·메이커 3곳의 "완성도" 표시가 같은 룰을 쓰는지 통일. 최우선 진입점: `maker-validation.ts` 라인별 룰 → `MakerEditor.tsx` 의 사용처 → `GameCard.tsx`/`buildMetadataFromRow`/`game-publish.ts`. 발견: 2026-04-27.
