...
- 같은 브라우저에서는 `/join/<code>` 진입 시 저장된 `mm_<sessionId>` 토큰을 먼저 확인하고 자동 복귀한다.
- 잠긴 슬롯도 이름 기반 `rejoin` 으로 복구할 수 있다.
- 참가 시 이름 입력란에 “재접속할 때도 같은 이름으로 확인한다”는 안내를 추가했다.
- GM 보드에 `재참가 허용` 버튼이 들어갔다.
- `재참가 허용` 이후 재입장 시에는 캐릭터의 진행 상태, 인벤토리, 라운드 진행, 투표 상태를 새 토큰으로 이어받는다.

## 현재 남은 것

- GM이 `재참가 허용`을 누른 뒤 기존 접속자 화면에 권한 교체 안내를 띄우는 UX
- 이름 기반 복구가 현재 정책상 충분한지, 필요 시 더 가벼운 운영 보강이 필요한지 검토
- 세션 복구 관련 스모크 테스트 항목을 정식 체크리스트로 문서화
...
- 실제 플레이 중 세션 안정성에 직접 영향이 있어 당시 우선순위를 높게 잡았고,
- 현재는 핵심 복구 흐름이 구현돼서 `운영 안내/권한 교체 UX` 보강 단계로 내려왔다.
