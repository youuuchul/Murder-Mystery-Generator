...
/** 분기 엔딩 스토리 패널 — 스토리 텍스트만 공개 (진범/검거 결과는 스토리에 포함) */
...
export interface PlayerViewProps {
  ...
  initialState?: PlayerSessionStateResponse | null;
  /** 서버 컴포넌트에서 읽은 세션 토큰 (쿠키 기반). 있으면 localStorage 조회 생략. */
  initialToken?: string | null;
}

export default function PlayerView({ initialState, initialToken }: PlayerViewProps = {}) {
  ...
  const [game, setGame] = useState<GamePackage | null>(initialState?.game ?? null);
  const [sharedState, setSharedState] = useState<SharedState | null>(initialState?.sharedState ?? null);
  const [sessionCode, setSessionCode] = useState(initialState?.sessionCode ?? "");
  const [sessionName, setSessionName] = useState(initialState?.sessionName ?? "");
  const [sessionMode, setSessionMode] = useState<SessionMode>(
    initialState?.sessionMode === "player-consensus" ? "player-consensus" : "gm"
  );
  const [isHost, setIsHost] = useState(initialState?.isSessionHost ?? false);
  const [sharedBoard, setSharedBoard] = useState<PlayerSharedBoardContent | null>(initialState?.sharedBoard ?? null);
  const [endedAt, setEndedAt] = useState<string | undefined>(initialState?.endedAt);
  const [inventory, setInventory] = useState<InventoryCard[]>(initialState?.playerState.inventory ?? []);
  const [myVotes, setMyVotes] = useState<Record<string, string>>(initialState?.myVotes ?? {});
  const [roundAcquired, setRoundAcquired] = useState<Record<string, number>>(initialState?.playerState.roundAcquired ?? {});
  const [roundVisited, setRoundVisited] = useState<Record<string, string[]>>(initialState?.playerState.roundVisitedLocations ?? {});
  ...
  const [showVictoryCondition, setShowVictoryCondition] = useState(false);
  ...
  useEffect(() => {
    // 서버에서 이미 쿠키로 토큰을 읽어 전달했다면 localStorage 조회 생략.
    // localStorage만 있는 레거시 사용자도 쿠키로 동기화해 다음 하드 리로드부터 SSR 가능.
    if (initialToken) {
      syncPlayerSessionCookieFromLocalStorage(sessionId);
      return;
    }
    ...
    if (!t) {
      ...
      setToken(t);
      // localStorage에만 있는 기존 사용자에게 쿠키 부여 (마이그레이션)
      syncPlayerSessionCookieFromLocalStorage(sessionId);
    }
  }, [sessionId, initialToken]);

  // 서버에서 initialState를 받았는지 여부. 받은 첫 렌더에선 fetch 생략.
  const initialStateConsumedRef = useRef(Boolean(initialState));

  useEffect(() => {
    if (!token || !sessionId) return;
    if (initialStateConsumedRef.current) {
      // 서버에서 받은 초기 상태를 이미 state에 반영했으므로 첫 fetch를 건너뛴다.
      // 이후 폴링/SSE가 상태를 최신으로 유지한다.
