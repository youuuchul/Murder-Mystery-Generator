  ...
  switch (task) {
    ...
    case "suggest_clues":
      return [
        ...
        "[본문]",
        "{제작자가 단서 카드 설명란에 그대로 붙여넣을 문장. 1~3문장, 서사 톤, 게임 메타 정보 금지. UUID/내부 ID 출력 금지: 캐릭터/장소는 반드시 이름(예: '드레이크 발터', '서재')으로 표기한다.}",
        "[근거]",
        "{왜 이 단서가 지금 필요한지, 어떤 기존 서사·단서·캐릭터와 연결되는지 1~3문장 요약. 운영 지시나 메타 설명 허용.}",
        "- 두 블록 라벨 외에는 섹션 마커를 추가하지 않는다.",
