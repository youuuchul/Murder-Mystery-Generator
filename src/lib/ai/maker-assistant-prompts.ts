  ...
  switch (task) {
    ...
    case "suggest_clues":
      return [
        ...
        "FINDING.detail 필드는 반드시 아래 두 블록 포맷으로 작성한다:",
        "[본문]",
        "→ 이 자리에 제작자가 단서 카드 설명란에 그대로 붙여넣을 문장을 적는다. 1~3문장, 서사 톤, 게임 메타 정보 금지. UUID/내부 ID 출력 금지: 캐릭터/장소는 반드시 이름(예: '드레이크 발터', '서재')으로 표기한다.",
        "[근거]",
        "→ 이 자리에 왜 이 단서가 지금 필요한지, 어떤 기존 서사·단서·캐릭터와 연결되는지 1~3문장으로 요약한다. 운영 지시나 메타 설명 허용.",
        "- 두 블록 라벨([본문], [근거]) 외에는 섹션 마커를 추가하지 않는다.",
        "- 위 안내문에 쓰인 '→', '{', '}' 같은 자리표시 기호는 실제 출력에 절대 포함하지 않는다. 본문 텍스트만 라벨 아래에 그대로 적는다.",
        "- 본문 블록에 발견 조건/획득 분기 같은 운영 규칙을 섞지 않는다. 그런 내용은 [근거] 블록에만 남긴다.",
        "- [본문] 블록의 문장은 반드시 평서문 종결형(~다/~었다/~하였다 등)으로 끝낸다. 명사형 마무리('~있음', '~함', '~됨')나 개조식 종결은 금지한다.",
      ].join("\n");
  ...
  switch (intent.profile) {
    ...
    case "descriptive_copy": {
      const base = [
        ...
        "- 모든 문장은 평서문 종결형(~다/~었다/~하였다 등)으로 끝낸다. 명사형 마무리('~있음', '~함', '~됨')나 개조식 종결은 금지한다.",
      ];

      // 장소 설명 타겟이면 해당 장소의 단서를 유추할 수 없게 "묘사만" 쓰도록 추가 가드를 건다.
      // 입력 데이터(단서 목록 등)는 맥락 참고용으로만 쓰고, 본문에 실물/표식/제목/해석을 드러내지 않는다.
      if (/장소/u.test(intent.targetLabel)) {
        base.push(
          "- 이 본문은 장소 설명이다. 감각과 배경 스토리로 몰입감을 올리는 묘사에만 집중한다.",
          "- 이 장소에 배치된 단서의 존재/위치/외형/제목/해석 힌트는 절대 포함하지 않는다. 컨텍스트의 clue 목록은 본문 방향성 참고용일 뿐 묘사 대상이 아니다.",
          "- '여기 어딘가에 쪽지가 있다', '수상한 물건이 보인다' 같은 방식의 발견 유도 문장도 금지한다. 탐색 여부는 플레이어가 조사 액션으로 판단할 몫이다.",
          "- 특정 아이템(책, 편지, 도구 등)이 단서에 해당하면 그 아이템을 본문에 지목하지 말고, 조사 전에도 자연스럽게 존재할 수 있는 가구·흔적·분위기로 대체한다.",
        );
      }
    ...
    case "timeline_plan":
      return [
        ...
        "- 컨텍스트 timeline 배열의 status 필드를 참고한다:",
        "  - status='inactive'는 제작자가 '이 시간대에는 등장/행동하지 않음'으로 확정한 상태이므로 새 ENTRY를 만들지 않는다.",
        "  - status='empty'는 아직 제작자가 결정하지 않은 슬롯이므로 서사상 의미 있다면 ENTRY를 제안해도 된다.",
        "  - status='filled'는 이미 입력된 상태이므로 기존 내용을 크게 덮어쓰지 말고 보강이나 정합성 제안이면 NOTES에 짧게 남긴다.",
