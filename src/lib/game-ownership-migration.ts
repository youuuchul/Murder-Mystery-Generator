 ...
 */
export async function migrateLocalGameOwnership(
  ...
  for (const metadata of await listGames()) {
    const game = await getGame(metadata.id);
    if (!game || !isGameOwner(game, fromOwnerId)) {
      continue;
    }

    await saveGame(reassignGameOwnership(game, toOwnerId, now));
